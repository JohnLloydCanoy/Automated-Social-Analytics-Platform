...
import ConnectSocial, { ConnectSocialInfoModal } from './ConnectSocial';
...
export default function SocialPlatformCard({ data, onRemove, onConnectionChange }: Props) {
    ...
    const handleSubmitInfo = async (info: any) => {
        setIsLoading(true);
        console.log("Social media info submitted:", info);
    ...
    return (
        <>
            ...
            <ConnectSocial 
                ...
                onConfirm={handleContinueToInfo} 
                platformName={data.name}
                isLoading={isLoading}
            />
            <ConnectSocialInfoModal
